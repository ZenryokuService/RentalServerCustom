/**
    This scripts from https://github.com/MilllerTime/
    Thanks for Caleb Miller
 */
...
//const messageStage = new Stage('message-canvas');
...
function printMessage(event) {
	var maxWidth = window.parent.screen.width;
	var maxHeight = window.parent.screen.height;
	var xPoint = Math.floor(Math.random()*(maxWidth-1)+1);
	var yPoint = Math.floor(Math.random()*(maxHeight-1)+1);
...
function togglePause(toggle) {
	...
	let newValue = typeof toggle === 'boolean'
		? toggle
		: !paused;
...
function toggleSound(toggle) {
	store.setState({
	 soundEnabled: typeof toggle === 'boolean'
	 	? toggle
...
function toggleMenu(toggle) {
	store.setState({ menuOpen: typeof toggle === 'boolean'
		? toggle
...
const helpContent = {
	...
	shellSize: {header: 'Shell Size',body: 'The size of the fireworks. Modeled after real firework shell sizes, larger shells have bigger bursts with more stars, and sometimes more complex effects. However, larger shells also require more processing power and may cause lag.'},
	quality: {header: 'Quality',body: 'Overall graphics quality. If the animation is not running smoothly, try lowering the quality. High quality greatly increases the amount of sparks rendered and may cause lag.'},
	skyLighting: {header: 'Sky Lighting',body: 'Illuminates the background as fireworks explode. If the background looks too bright on your screen, try setting it to "Dim" or "None".'},
	scaleFactor: {header: 'Scale',body: 'Allows scaling the size of all fireworks, essentially moving you closer or farther away. For larger shell sizes, it can be convenient to decrease the scale a bit, especially on phones or tablets.'},
	autoLaunch: {header: 'Auto Fire',body: 'Launches sequences of fireworks automatically. Sit back and enjoy the show, or disable to have full control.'},
	finaleMode: {header: 'Finale Mode',body: 'Launches intense bursts of fireworks. May cause lag. Requires "Auto Fire" to be enabled.'},
	hideControls: {header: 'Hide Controls',body: 'Hides the translucent controls along the top of the screen. Useful for screenshots, or just a more seamless experience. While hidden, you can still tap the top-right corner to re-open this menu.'},
	fullscreen: {header: 'Fullscreen',body: 'Toggles fullscreen mode.'},
	longExposure: {header: 'Open Shutter',body: 'Experimental effect that preserves long streaks of light, similar to leaving a camera shutter open.'}
};
...
Object.keys(nodeKeyToHelpKey).forEach(nodeKey => {
	...
	appNodes[nodeKey].addEventListener('click', () => {store.setState({ openHelpTopic: helpKey });});
});
...
function handlePointerStart(event) {
	...
	if (event.y < btnSize) {
		...
		if (event.x > mainStage.width/2 - btnSize/2 && event.x < mainStage.width/2 + btnSize/2) {toggleSound();return;}
...
function handleKeydown(event) {
    // printMessage
    if (event.key === 'q') {printMessage(event);}
	// P
	...
	burst(x, y) {
		...
		if (typeof this.color === 'string') {
			color = this.color === 'random'
				? null // falsey value creates random color in starFactory
				: this.color;
...
// Kick things off.
function setLoadingStatus_message(status) {
	document.querySelector('#message-canvas').textContent = status;
}

// CodePen profile header doesn't need audio, just initialize.
if (IS_HEADER) {
	init();
	setLoadingStatus_message('Lighting Fuses');
